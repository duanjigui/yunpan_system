...
<form>
		<div class="dataSerizeals">
			<table>
				...
				<tr>
					<td>消息级别:</td>
					<td><input type="text" name="mes_leavl" value="${message.mes_leavl}"/></td>
				</tr>
				<tr>
					<input type="hidden" name="mes_send_user_name" value="${message.mes_send_user_name}"/>
					<td>消息接收者:</td>
					<td>
				...
				<tr>
					<input type="hidden" name="mes_send_group_name" value="${message.mes_send_group_name}"/>
					<td>消息发送群组:</td>
					<td>
				...
				<tr>
					...
					<td>
						<select name="mes_is_delete">
							<c:if test="${message.mes_is_delete eq '0'}">
								<option value="0" selected="selected">启用</option>
								<option value="1">停用</option>
							</c:if>
							<c:if test="${message.mes_is_delete eq '1'}">
				...
				<tr>
					...
					<td><textarea rows="10" cols="70" name="mes_content" >${message.mes_content}</textarea> </td>
				</tr>
			</table>	
		</div>
	...
	$(function(){
		...
		$.post("${pageContext.request.contextPath}/manager/user/all",null,function(data){
			if(data != null && data.length >0){
					for(var i=0;i<data.length;i++){
						var $node="";
						 if(data[i].user_id == "${message.mes_send_user_id}"){
		...
		$.post("${pageContext.request.contextPath}/manager/orianzation/listAll",null,function(data){
			if(data != null && data.length >0){
				for(var i=0;i<data.length;i++){
					var $node="";
					if(data[i].orange_id== "${message.mes_send_group_id}"){
						var $node="<option value="+data[i].orange_id+" selected='selected'>"+data[i].orange_name+"</option>";
					   }else{
		...
		$(".ok").click(function(){
			...
			$.post("${pageContext.request.contextPath}/message/update",$("form").serialize(),function(data){
