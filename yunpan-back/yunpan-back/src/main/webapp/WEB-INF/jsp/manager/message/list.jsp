	...
	var dialog;
        ...
        function itemclick(item)
        {
            if(item.text == "修改"){
				...
				//向服务端发送要删除的数据
				var contents="<form action='${pageContext.request.contextPath}/message/delete' method='post'>";
        ...
        $(function ()
        {
			...
			for(var i=0;i<data.Rows.length;i++){
			  ...
			  var status=	data.Rows[i].mes_is_delete;
			  if(status == 0){
				  data.Rows[i].mes_is_delete="启用"
			  }else{
				  data.Rows[i].mes_is_delete="停用"
				}
            ...
            $("#maingrid").ligerGrid({
                ...
                pageSize :10,
                columns: [
                { display: '消息id', name: 'message_id', align: 'center', width: 60, minWidth: 60 },
                { display: '消息接收者', name: 'mes_send_user_name', minWidth: 50 },
                { display: '消息内容', name: 'mes_content', minWidth: 50 },
                { display: '消息级别', name: 'mes_leavl', minWidth: 50 },
                { display: '消息状态', name: 'mes_is_delete', minWidth: 50 },
                { display: '消息发送的群组', name: 'mes_send_group_name', minWidth: 50 },
