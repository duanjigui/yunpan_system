	...
	var dialog;
        ...
        function itemclick(item)
        {
            if(item.text == "修改"){
				...
				//向服务端发送要删除的数据
				var contents="<form action='${pageContext.request.contextPath}/manager/catelogtype/delete' method='post'>";
        ...
        $(function ()
        {
            var data=${catelogeType};

            //时间格式化
			for(var i=0;i<data.Rows.length;i++){
			   var status= data.Rows[i].status;
				if(status == 0){
					data.Rows[i].status="停用";
				}else{
            ...
            $("#maingrid").ligerGrid({
                ...
                pageSize :10,
                columns: [
                { display: '栏目id', name: 'catelog_type_id', align: 'center', width: 60, minWidth: 60 },
                { display: '栏目名称', name: 'catelog_type_name', minWidth: 50 },
                ...
                { display: '父内容类型名称', name: 'parent_name', minWidth: 50 }
