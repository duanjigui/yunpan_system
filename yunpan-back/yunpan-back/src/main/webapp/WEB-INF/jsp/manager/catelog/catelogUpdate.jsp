...
<body>
<form action="${pageContext.request.contextPath}/manager/catelog/update" method="post">
		<div class="dataSerizeals">
			<table>
				<input type="hidden"  name="catelog_id" value="${cateloge.catelog_id}" />
				<tr>
					<td>标题:</td>
					<td><input type="text" name="title" value="${cateloge.title}"/></td>
				</tr>
				<tr>
					<td>状态:</td>
					<td>
						<select name="is_delete">
							<c:if test="${cateloge.is_delete == 0}">
								<option value="0" selected='selected'>禁用</option>
								<option value="1">启用</option>
							</c:if>
							<c:if test="${cateloge.is_delete == 1}">
				...
				<tr>
					...
					<td><textarea rows="10" cols="70" name="content" >${cateloge.content} </textarea> </td>
				</tr>
			</table>	
		</div>
			...
			$.post("${pageContext.request.contextPath}/manager/catelogtype/all",null,function(data){
					if(data != null && data.length >0){
						for(var i=0;i<data.length;i++){
							var $node="";
							if(data[i].catelog_type_id=="${cateloge.catelog_type_id}"){
